...
#   rate so guest checkouts are handled gracefully instead of crashing.
def quote_shipping(order: dict) -> dict:
    customer = order["customer"]

    # Guest checkouts arrive with customer=null; fall back to the standard
    # shipping rate since no address is available to resolve a tier.
    if customer is None:
        logger.warning(
            json.dumps({"event": "quote.guest_checkout", "orderId": order.get("id")})
        )
        return {
