...
# ROOT CAUSE was: guest checkouts send `"customer": null`, so `order["customer"]`
#   was None and subscripting it raised `TypeError: 'NoneType' object is not
#   subscriptable`.
def quote_shipping(order: dict) -> dict:
    customer = order.get("customer")  # use .get() — safe even if key is absent
    if not customer:
...
def handler(event, context):
    ...
    Guest checkouts arrive as:
        {"id": "ord_43", "customer": null}   <-- now handled gracefully
    """
